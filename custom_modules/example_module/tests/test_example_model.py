...
class TestExampleModel(TransactionCase):
    ...
    def setUp(self):
        ...
        # Create test tags
        self.tag_test = self.tag_model.create({
            'name': 'Test Tag',
            'color': 1
        })
    ...
    def test_search_and_filtering(self):
        ...
        # Create test records
        example1 = self.example_model.create({
            'name': 'Example 1',
            'value': 100.0,
            'state': 'draft'
        })
        
        example2 = self.example_model.create({
            'name': 'Example 2',
            'value': 200.0,
            'state': 'confirmed'
        })
    ...
    def test_required_fields(self):
        ...
        with self.assertRaises(Exception):
            self.example_model.create({
                'value': 100.0
            })

    def test_ordering(self):
        """Test record ordering."""
        example_z = self.example_model.create({
            'name': 'Z Example',
            'value': 100.0
        })
        
        example_a = self.example_model.create({
            'name': 'A Example',
            'value': 100.0
        })
